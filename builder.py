...
def get_hash(dir: Path):
    proc_bytes = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=dir)
...
def check_build_site():
    ...
    subprocess.run("git fetch --all && git reset --hard origin/master && git pull", cwd=ROOT_DIR, shell=True)
    ...
    if len(sys.argv) > 1:
        ...
        match sys.argv[1].lower().strip():
            ...
            case "all":
                ...
                changed = resume_changed or site_changed
            case "site":
                changed = check_build_site()
            case "resume":
                changed = check_build_resume()

        if changed:
            # move files to nginx locations
            pass
